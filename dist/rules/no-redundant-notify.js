    ...
    create: (context) => {
        ...
        return {
            "ExpressionStatement[expression.callee.property.name=/^(complete|error)$/] + ExpressionStatement[expression.callee.property.name=/^(next|complete|error)$/]": (node) => {
                ...
                if (!(0, etc_1.isBlockStatement)(parent) && !(0, etc_1.isProgram)(parent)) {
                    return;
                }
                const { body } = parent;
                ...
                if ((0, etc_1.isCallExpression)(expression)) {
                    const { callee } = expression;
                    if ((0, etc_1.isMemberExpression)(callee)) {
                        const { property } = callee;
                        if ((0, etc_1.isIdentifier)(property)) {
                            context.report({
                                messageId: "forbidden",
                                node: property,
                            });
...
function getExpressionText(expressionStatement, sourceCode) {
    if (!(0, etc_1.isCallExpression)(expressionStatement.expression)) {
        return undefined;
    }
    const callExpression = expressionStatement.expression;
    if (!(0, etc_1.isMemberExpression)(callExpression.callee)) {
        return undefined;
    }
